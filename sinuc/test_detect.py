...
# Генерация данных и детекция аномалий
sg = SineGenerator(noise=noise, n_points=n_points, start=start, end=end, 
                   amplitude=amplitude, frequency=frequency, 
                   anomaly_count=anomaly_count, anomaly_scale=anomaly_scale)
sg.generate()

anomalies_stat = detect(sg.df)  # статистический метод
anomalies_ml = detect_ml(sg.df)  # метод машинного обучения
true_anomalies = sg.df['is_anomaly']  # истинные аномалии
...
with main_col:
    ...
    with col1:
        ...
        col1.metric("Аномалий выявлено", len(sg.df.loc[anomalies_stat, 'x']), 
                   delta=len(sg.df.loc[anomalies_stat, 'x']) - len(sg.df.loc[anomalies_ml, 'x']))
    ...
    with col2:
        ...
        col2.metric("Аномалий выявлено", len(sg.df.loc[anomalies_ml, 'x']), 
                   delta=len(sg.df.loc[anomalies_ml, 'x'])-len(sg.df.loc[anomalies_stat, 'x']))

st.divider()

# Метрики качества для ML метода
...
c1.metric("ML Precision", format(precision, ".4f"))
c2.metric("ML recall", format(recall, ".4f"))
c3.metric("ML f1_score", format(f1_score, ".4f"))

# Метрики качества для статистического метода
